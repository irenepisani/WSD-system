...
def row_comparison (lines_gold,lines_auto):
	...
	pos=["NOUN", "ADJ", "ADV", "VERB", "PROPN"]

	#prendo una frase alla volta con stesso indice
	for i, j  in zip(lines_gold,lines_auto, k):
	...
	return evaluation_metric, difference
...
def get_Fmeasure  (p1,p2,r1,r2):	#calcolo Fmeasure
	Fmeasure = (2*(p1+p2)*(r1+r2))/(p1+p2+r1+r2)
	return Fmeasure 

def get_accuracy(dictionary1, dictionary2): #calcolo accuracy 
...
def main (file1,file2,file3):
	...
	Fmeasure_missing = get_FmeasureTOT(precision_missing_psc, recall_missing_iwn)
